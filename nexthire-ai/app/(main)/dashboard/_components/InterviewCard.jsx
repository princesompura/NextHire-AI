...
function InterviewCard({ interview, viewDetail = false }) {
  ...
  const onSend = () => {
    window.location.href =
      "mailto:accounts@gmail.com?subject=Interview Link&body=Interview Link:" + url;
  };
    ...
    <div className="p-5 bg-blue-50 rounded-lg border mt-5">
      ...
      {!viewDetail ? (
        <div className="flex gap-3 mt-2">
          ...
          <Button
            className="bg-blue-200 hover:bg-blue-300 text-black cursor-pointer"
            onClick={onSend}
          >
            <Send className="text-black" /> Mail To
          </Button>
        </div>
      ) : (
        <Link href={`/Scheduled-interview/${interview?.interview_id}/details`}>
          <Button className="mt-5 w-full bg-blue-200 hover:bg-blue-300" variant="outline">
